...
int main(int argc, char * argv[]) {
    ...
    /* раскладываем commsize процессов на 2 сомножителя */
    ...
    /* организовываем все процессы в 2D сетку
    новый коммуникатор cartcomm для всех процессов вида (x, y) */
    MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periodic, 0, &cartcomm);
    int coords[2];
    MPI_Cart_coords(cartcomm, rank, 2, coords);
    int rankx = coords[0]; /* координаты процесса по x */
    int ranky = coords[1]; /* координаты процесса по y */
    int rows, cols;
    ...
    /* получаем номера процессов слева и справа, сверху и снизу, если их нет — записано MPI_PROC_NULL */
    int left, right, top, bottom;
    /* указываем куда нужно сметиться, чтобы получить номера процессов-соседей */
    MPI_Cart_shift(cartcomm, 0, 1, &left, &right);
    MPI_Cart_shift(cartcomm, 1, 1, &top, &bottom);
    // Left and right borders type
    MPI_Datatype col;
    /* формируем тип данных для передачи стобца с регулярным шагом */
    MPI_Type_vector(ny, 1, nx + 2, MPI_DOUBLE, & col);
    MPI_Type_commit( & col);
    // Top and bottom borders type
    MPI_Datatype row;
    /* формируем тип данных для передачи строки */
    MPI_Type_contiguous(nx, MPI_DOUBLE, & row);
    MPI_Type_commit( & row);
    MPI_Request reqs[8]; /* в худшем случае 4 соседа (send & recv) */
    double thalo = 0;
    double treduce = 0;
    int niters = 0;
    for (;;) {
        ...
        for (int i = 1; i <= ny; i++) {
            for (int j = 1; j <= nx; j++) {
                /* формируем в новой сетке элемент[i][j] по схеме крест из старой сетки*/
        ...
        for (int i = 1; i <= ny; i++) {
            for (int j = 1; j <= nx; j++) {
                int ind = IND(i, j);
                /* вычисляем максимум среди модулей разности значений старой и новой сеток */
        ...
        local_newgrid = p;
        treduce -= MPI_Wtime();
        /* находим максимум maxdiff по всем процессам */
        MPI_Allreduce(MPI_IN_PLACE, &maxdiff, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
        treduce += MPI_Wtime();
        if (maxdiff < EPS) {
            break;
        }
        // Обмен теневыми ячейками: T = 4 * (a + b * (rows / py)) + 4 * (a + b * (cols / px))
        thalo -= MPI_Wtime();
